...
testSize()
{
  ...
  # Copy ${F}.0 --> ${F}.1
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --size -o
  ...
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --size -o 2>/dev/null
  assertEquals "${FUNCNAME[0]}:$LINENO" 2 $?

  # If $F is empty (i.e. there is $F.0 and $F.1)
  # $F.1 is copied to ${F}.0 with -f option.
  echo >${F}.0
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" -f --size -o
...
testMD5() {
  ...
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --md5="${SUM0}" -o
  assertEquals 0 $?
  SUM1=$(md5sum "${F}.1" | cut -f 1 -d ' ')
  assertEquals "${SUM0}" "${SUM1}"

  # If the file $F.1 exists, next call must be an error
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --md5="${SUM0}" -o 2>/dev/null
  assertEquals 2 $?
  ...
  # now with -f
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --md5="${SUM0}" -f -o 2>/dev/null
  SUM1=$(md5sum "${F}.1" | cut -f 1 -d ' ')
  assertEquals "${SUM0}" "${SUM1}"
}
...
testRank() {
  ...
  SUM0=$(md5sum ${F}.0 | cut -f 1 -d ' ')
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --rank=0 -o
  assertEquals 0 $?
  SUM1=$(md5sum "${F}.1" | cut -f 1 -d ' ')
  assertEquals "${SUM0}" "${SUM1}"
  ...
  ### If the file $F.1 exists, next call must be an error
  ls test_tmp/
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --rank=0 -o 2>/dev/null
  assertEquals 2 $?
  ...
  # Copy rank 1 --> rank 0
  mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --rank=1 -f -o 2>/dev/null
  echo "(3)"
  md5sum test_tmp/*
  SUM0=$(md5sum ${F}.0 | cut -f 1 -d ' ')
...
testSmallChunk() {
  ...
  for CHUNK_SIZE in 1 2 3 4 5; do
      rm -f ${F}.1
      SUM0=$(md5sum ${F}.0 | cut -f 1 -d ' ')
      mpiexec -n 2 python mpicpy/mpicpy.py "${F}.{rank}" --rank=0 -c $CHUNK_SIZE -o
      assertEquals 0 $?
      SUM1=$(md5sum "${F}.1" | cut -f 1 -d ' ')
      assertEquals "${SUM0}" "${SUM1}"
  done
